...
func (r *BookingRepository) CreatePending(ctx context.Context, booking *domain.Booking) error {
...
func (r *BookingRepository) GetByToken(ctx context.Context, token string) (*domain.Booking, error) {
...
func (r *BookingRepository) UpdateStatus(ctx context.Context, token string, status domain.BookingStatus) (*domain.Booking, error) {
...
func (r *BookingRepository) ExpirePendingBefore(ctx context.Context, deadline time.Time) ([]domain.Booking, error) {
...
func (r *BookingRepository) ReleaseSeat(ctx context.Context, flightID int64) error {
	cmd, err := r.db.Exec(ctx, `UPDATE flights SET available_seats = available_seats + 1, updated_at = now() WHERE id=$1`, flightID)
	if err != nil {
		return err
	}
